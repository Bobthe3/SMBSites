...
const navigation = [
  { name: 'Home', href: '/keekuzcom' },
  { name: 'Menu', href: '/keekuzcom/menu' },
  { name: 'Catering', href: '/keekuzcom/catering' },
  { name: 'About', href: '/keekuzcom/about' },
  { name: 'Contact', href: '/keekuzcom/contact' },
];
    ...
    <header className="bg-white/95 backdrop-blur-sm shadow-sm sticky top-0 z-50">
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <Link href="/keekuzcom" className="flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navigation.map((item) => {
              const isActive = pathname === item.href || (item.href !== '/keekuzcom' && pathname.startsWith(item.href));
              return (
                <Link
                  key={item.name}
                  href={item.href}
